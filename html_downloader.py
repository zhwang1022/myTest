...
class HtmlDownLoader(object):
    def download(self, url, retry_count=3, headers=None, proxy=None, data=None):
        ...
        try:
            ...
            cookie = cookiejar.CookieJar()  # 声明 CookieJar 类
            cookie_process = request.HTTPCookieProcessor(cookie)  # 构建一个 handler
            opener = request.build_opener()
            if proxy:
                proxies = {urlparse(url).scheme: proxy}  # urlparse主要对url进行解析，对url按照一定格式进行拆分，拼接。
